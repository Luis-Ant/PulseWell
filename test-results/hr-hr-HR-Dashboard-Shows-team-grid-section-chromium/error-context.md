...
- Name: hr/hr.spec.ts >> HR Dashboard >> Shows team grid section
- Location: tests/hr/hr.spec.ts:36:7
...
Locator: locator('section').filter({ hasText: 'Equipos' }).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ hasText: 'Equipos' }).first()
